...
def get_user_preferences_from_history(email: Optional[str], limit_messages: int = 5) -> List[str]:
    ...
    if supabase:
        try:
            ...
            # normalize string stored as JSON
            if isinstance(hist, str):
                try:
                    hist = json.loads(hist)
                except Exception:
    ...
    # collect last user messages (non-nova)
    for _, m in items:
    ...
    for msg in messages:
        ...
        for k in KEYWORD_CATEGORY.keys():
            if k in t and k not in prefs:
                prefs.append(k)
        # fallback: naive nouns (short tokens)
...
def chat(req: ChatReq):
    ...
    # If user is answering "yes" to followup, try to continue last conv
    ...
    # Build ordered topic list: preferences first
