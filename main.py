...
DEFAULT_RSS_FEEDS = [ "http://feeds.bbci.co.uk/news/rss.xml", "http://rss.cnn.com/rss/edition.rss", "https://www.theguardian.com/world/rss", "https://feeds.reuters.com/reuters/topNews", "https://feeds.npr.org/1001/rss.xml", # tech "https://techcrunch.com/feed/", "https://www.theverge.com/rss/index.xml", "https://feeds.arstechnica.com/arstechnica/index", # business "https://feeds.a.dj.com/rss/RSSWorldNews.xml", "https://feeds.reuters.com/reuters/businessNews", # space / science "https://www.nasa.gov/rss/dyn/breaking_news.rss", "https://www.space.com/feeds/all", "https://www.sciencedaily.com/rss/top/science.xml", ]

CATEGORY_FEEDS = { "space": [ "https://www.nasa.gov/rss/dyn/breaking_news.rss", "https://www.space.com/feeds/all", "https://www.spaceflightnow.com/launch-feed/", ], "tech": [ "https://techcrunch.com/feed/", "https://www.theverge.com/rss/index.xml", "https://feeds.arstechnica.com/arstechnica/index", ], "business": [ "https://feeds.a.dj.com/rss/RSSWorldNews.xml", "https://feeds.reuters.com/reuters/businessNews", ], }

RSS_FEEDS = (EXTRA_RSS.split(",") if EXTRA_RSS else []) + DEFAULT_RSS_FEEDS

quick keyword -> category (kept for fallback)

KEYWORD_CATEGORY = { "nasa": "space", "space": "space", "spacex": "space", "jwst": "space", "ai": "tech", "google": "tech", "apple": "tech", "markets": "business", }

---------------- MODELS / INPUT ----------------

class ChatReq(BaseModel): user_email: Optional[str] = None message: str conversation_name: Optional[str] = None

class GetChatReq(BaseModel): email: str conversation_name: str

class RenameReq(BaseModel): user_email: str old_name: str new_name: str

class DeleteReq(BaseModel): user_email: str conversation_name: str
...
def save_sqlite_chat_history(email: str, hist: List[dict]): cur = DB.cursor() cur.execute("UPDATE users SET chat_history = ? WHERE email = ?", (json.dumps(hist), email)) DB.commit() return True

def append_new_conversation(email: str, conv_name: str, messages: List[dict]): ensure_user_row_sqlite(email) hist = fetch_sqlite_chat_history(email) or [] hist.append({conv_name: messages}) save_sqlite_chat_history(email, hist) return True

def find_conversation_index(hist: List[dict], conv_name: str) -> int: for i, obj in enumerate(hist): if isinstance(obj, dict) and conv_name in obj: return i return -1

def append_message_to_conversation(email: str, conv_name: str, sender: str, text: str): ensure_user_row_sqlite(email) hist = fetch_sqlite_chat_history(email) or [] idx = find_conversation_index(hist, conv_name) msg = {"sender": sender, "text": text, "ts": datetime.utcnow().isoformat()+"Z"} if idx >= 0: hist[idx][conv_name].append(msg) else: hist.append({conv_name: [msg]}) save_sqlite_chat_history(email, hist) return True

def rename_sqlite_conversation(email: str, old_name: str, new_name: str): hist = fetch_sqlite_chat_history(email) or [] idx = find_conversation_index(hist, old_name) if idx < 0: return False hist[idx] = {new_name: hist[idx][old_name]} save_sqlite_chat_history(email, hist) return True

def delete_sqlite_conversation(email: str, conv_name: str): hist = fetch_sqlite_chat_history(email) or [] new_hist = [obj for obj in hist if not (isinstance(obj, dict) and conv_name in obj)] changed = len(new_hist) < len(hist) if changed: save_sqlite_chat_history(email, new_hist) return changed
...
def extract_topic_from_message(msg: str) -> str: if not msg: return "news" m = re.search(r"(?:about|on|regarding|regarding:|regarding\s)([^?.!]+)", msg, re.I) if not m: # try phrase after 'news' or 'latest' m = re.search(r"(?:news|latest|updates?)\s(?:about\s)?([^?.!]+)", msg, re.I) if m: topic = m.group(1).strip() # trim filler like 'today' 'please' at end topic = re.sub(r"\b(today|please|now|for my project|for my|for)\b", "", topic, flags=re.I).strip() # crop length return topic[:120].strip() # fallback: take important nouns by removing greetings and short verbs cleaned = re.sub(r"\b(hey|hi|hello|buddy|please|can i|could you|i want|i'd like|i want to)\b", "", msg, flags=re.I) cleaned = cleaned.strip() if len(cleaned) > 0 and len(cleaned) < 140: return cleaned return "news"

Generate a concise conversation title using Gemini (<=4 words ideally)

def generate_conversation_name(user_message: str) -> str: base_fallback = re.sub(r"[^\w\s-]", "", user_message)[:40].strip() prompt = ( "You are a helpful assistant. Create a short, descriptive conversation title (max 4 words) suitable as a chat/conversation name. " f"Make it concise and human-friendly. Message: {user_message}\n\nTitle:" ) try: # try several SDK entrypoints gracefully if hasattr(genai, "Client"): client = genai.Client() resp = client.models.generate_content(model=MODEL_NAME, contents=prompt) text = getattr(resp, "text", None) if not text and hasattr(resp, "output") and isinstance(resp.output, (list, tuple)) and resp.output: text = getattr(resp.output[0], "content", None) or getattr(resp.output[0], "text", None) if text: s = text.strip().splitlines()[0] s = re.sub(r"["']", "", s).strip() s = re.sub(r"\s+", " ", s) return s[:60] except Exception: pass try: if hasattr(genai, "generate_content"): resp = genai.generate_content(model=MODEL_NAME, prompt=prompt) text = getattr(resp, "text", None) if text: s = text.strip().splitlines()[0] s = re.sub(r"["']", "", s).strip() s = re.sub(r"\s+", " ", s) return s[:60] except Exception: pass # hard fallback: take first 3-4 words parts = re.findall(r"\w+", base_fallback) title = " ".join(parts[:4]) or "chat" return title
...
def search_rss_for_topic(topic: str, max_items=30, category: Optional[str]=None): topic_l = (topic or "").lower().strip() found = [] try: feeds_to_check: List[str] = [] if category and category in CATEGORY_FEEDS: feeds_to_check += CATEGORY_FEEDS[category] for u in RSS_FEEDS: if u not in feeds_to_check: feeds_to_check.append(u)
...
def summarize_article(article_text: str, headline: str, user_message: str): try: prompt = ( "You are Nova — a friendly, concise AI news reporter.\n" "Summarize the article below in 2-4 short paragraphs with clear facts. " "Then produce one short tailored follow-up question the user might want next (1 sentence).\n\n" f"User message: {user_message}\n\n" f"Headline: {headline}\n\n" f"Article:\n{article_text}\n\n" "Summary:" ) # try multiple SDK shapes (robust) try: if hasattr(genai, "Client"): client = genai.Client() resp = client.models.generate_content(model=MODEL_NAME, contents=prompt) text = getattr(resp, "text", None) if not text and hasattr(resp, "output") and isinstance(resp.output, (list, tuple)) and resp.output: text = getattr(resp.output[0], "content", None) or getattr(resp.output[0], "text", None) if text: return text.strip() except Exception as e: print("genai.Client() call failed:", e) try: if hasattr(genai, "GenerativeModel"): model = genai.GenerativeModel(MODEL_NAME) for call_args in ( (prompt,), {"prompt": prompt}, {"contents": prompt} ): try: resp = model.generate_content(*call_args) if isinstance(call_args, tuple) else model.generate_content(**call_args) text = getattr(resp, "text", None) if text: return text.strip() except Exception: continue except Exception as e: print("GenerativeModel() call failed:", e) try: if hasattr(genai, "generate_content"): resp = genai.generate_content(model=MODEL_NAME, prompt=prompt) text = getattr(resp, "text", None) if text: return text.strip() except Exception as e: print("genai.generate_content() call failed:", e) # fallback extractive snippet paragraphs = article_text.split("\n\n") short = "\n\n".join(paragraphs[:2]).strip() fallback = (f"⚠️ Couldn't get a GenAI summary (SDK mismatch or quota). " f"Here is an extract:\n\n{short}\n\nWould you like me to try again or open the original link?") return fallback except Exception as e: print("Gemini summarizer final error:", e) traceback.print_exc() return "⚠️ Sorry — error while generating summary: Gemini unavailable."

---------------- fallback topic summarizer (news not found) ----------------

def summarize_topic_fallback(topic: str, user_message: str): prompt = ( "You are Nova — a friendly, concise AI news assistant.\n" "The user asked about this topic and I couldn't find direct articles in feeds.\n" "Provide a short chatty but factual summary (2-4 short paragraphs) of what is known about the topic right now, " "and one short question or suggestion the user might want next.\n\n" f"User message: {user_message}\n\n" f"Topic: {topic}\n\n" "Summary:" ) try: if hasattr(genai, "Client"): client = genai.Client() resp = client.models.generate_content(model=MODEL_NAME, contents=prompt) text = getattr(resp, "text", None) if not text and hasattr(resp, "output") and isinstance(resp.output, (list,tuple)) and resp.output: text = getattr(resp.output[0], "content", None) or getattr(resp.output[0], "text", None) if text: return text.strip() except Exception as e: print("fallback genai.Client failed:", e) try: if hasattr(genai, "GenerativeModel"): model = genai.GenerativeModel(MODEL_NAME) for call_args in ( (prompt,), {"prompt": prompt}, {"contents": prompt} ): try: resp = model.generate_content(*call_args) if isinstance(call_args, tuple) else model.generate_content(**call_args) text = getattr(resp, "text", None) if text: return text.strip() except Exception: continue except Exception as e: print("fallback GenerativeModel failed:", e) try: if hasattr(genai, "generate_content"): resp = genai.generate_content(model=MODEL_NAME, prompt=prompt) text = getattr(resp, "text", None) if text: return text.strip() except Exception as e: print("fallback genai.generate_content failed:", e) return (f"I couldn't find direct articles in my feeds, but here's a short summary of {topic} " "based on general knowledge. I can keep watching and alert you to updates if you'd like.")

---------------- intent heuristics (simpler + topic extraction) ----------------

NEWS_INTENT_KEYWORDS = ["news","update","updates","latest","breaking","report","released","release","new","any updates"] GREETINGS_RE = re.compile(r"\b(hi|hello|hey|hiya|greetings)\b", re.I)

def decide_need_news(user_message: str): msg = (user_message or "").strip() if not msg: return False, None, "" # greeting-only and short -> no news if len(msg) < 60 and GREETINGS_RE.search(msg) and not any(k in msg.lower() for k in NEWS_INTENT_KEYWORDS): return False, None, msg low = msg.lower() if any(k in low for k in NEWS_INTENT_KEYWORDS): topic = extract_topic_from_message(msg) return True, None, topic # If user explicitly asks about latest <topic> without keyword, still check 'about' patterns topic_try = extract_topic_from_message(msg) if topic_try and topic_try.lower() not in ("news","please") and len(topic_try) > 2: return True, None, topic_try return False, None, msg

---------------- MAIN /chat endpoint ----------------

@app.post("/chat") def chat(req: ChatReq): user_message = (req.message or "").strip() if not user_message: raise HTTPException(status_code=400, detail="message required") email = (req.user_email or "").strip().lower() or None conv_name_in = (req.conversation_name or "").strip() or None
